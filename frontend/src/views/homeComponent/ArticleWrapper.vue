...
function calculateLayout() {
  ...
  const children = Array.from(wrapperRef.value.childNodes).filter((node) => node.nodeType === 1)
...
<template>
  <div ref="wrapperRef" class="article-wrapper">
    <div class="active-item" v-for="article in articleList" :key="article.article_id">
      <ArticleItem
        :title="article.title"
        :author="article.author.username"
        :author-avatar="article.author.image"
        :likes="article.likes"
        :created-at="article.createdAt"
        :cover-image="article.image.length > 0 ? article.image[0] : ''"
        @click="openDetail(article.article_id)"
      />
    </div>
  </div>
  <ArticleDetailModal v-if="showDetail" :article-id="currentId" @close="showDetail = false" />
</template>
